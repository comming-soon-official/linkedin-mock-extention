...
const generateSVGIcon = `<svg width="16" height="14" viewBox="0 0 16 14" fill="none" xmlns="http://www.w3.org/2000/svg">
...
export const addSVGIconToInputBox = (element: HTMLElement): void => {
  if (element.querySelector(".custom-svg-icon")) return;
  ...
  icon.innerHTML = generateSVGIcon;

  icon.addEventListener("mouseover", () => {
    icon.style.backgroundColor = "#DFE1E7";
  });
  ...
  icon.addEventListener("click", () => {
    useUniversalStore.getState().setIsModalOpen(true);
  });
...
export const removeSVGIconFromInputBox = (element: HTMLElement): void => {
  const icon = element.querySelector(".custom-svg-icon");
  if (icon) {
    icon.remove();
  }
};

export function insertText(contentEditableElement: HTMLElement, text: string) {
  ...
  // Simulate a keypress event to make LinkedIn register the input
  ...
  // Simulate the change/input event to make sure LinkedIn catches the update
